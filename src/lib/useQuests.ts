...
export function useQuests(userId?: string, domainId?: string) {
  ...
  const api = useMemo(() => {
    return {
      ...
      async update(id: string, patch: Partial<Pick<Quest, 'title'|'summary'|'status'|'domainId'>> & { dueDate?: Date | null }) {
        ...
        if (patch.dueDate !== undefined) payload.dueDate = patch.dueDate ? Timestamp.fromDate(patch.dueDate) : null
