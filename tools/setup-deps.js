...
if (!auto) {
  if (missingRuntime.length) {
    const cmd =
      pm === "pnpm" ? `pnpm add ${missingRuntime.join(" ")}`
        : pm === "yarn" ? `yarn add ${missingRuntime.join(" ")}`
  ...
  if (missingDev.length) {
    ...
    const withPins = missingDev.map((d) =>
      VERSION_PINS[d] ? `${d}@${VERSION_PINS[d]}` : d
    );
    const cmd =
      pm === "pnpm" ? `pnpm add -D ${withPins.join(" ")}`
        : pm === "yarn" ? `yarn add -D ${withPins.join(" ")}`
